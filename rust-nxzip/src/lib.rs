...
//!
//! ```rust
//! use nxzip::engine::{Compressor, Decompressor, CompressionAlgorithm, SPECore};
//! use nxzip::crypto::{IntegratedEncryptor, IntegratedDecryptor, EncryptionAlgorithm};
//! use nxzip::formats::enhanced_nxz::EnhancedNxzFile;
//!
//! # tokio_test::block_on(async {
//! // データ準備
//! let data = b"Hello, NXZip Enhanced!";
//!
//! // SPE + 暗号化 + 圧縮の統合処理
...
//!
//! // 暗号化（SPE変換込み）
//! let encryption_result = encryptor.encrypt(data).unwrap();
//!
//! // 圧縮
//! let compressor = Compressor::new(CompressionAlgorithm::Zstd);
//! let compressed = compressor.compress(&encryption_result.encrypted_data).unwrap();
//!
//! // 強化NXZファイル作成
...
// 頻繁に使用される型の再エクスポート
pub use engine::{Compressor, Decompressor, CompressionAlgorithm, SPECore, SPEParameters, StructureLevel};
pub use crypto::{Encryptor, Decryptor, EncryptionAlgorithm, IntegratedEncryptor, IntegratedDecryptor, KeyDerivationFunction};
pub use formats::nxz::NxzFile;
pub use formats::nxz_sec::{NxzSecFile, KdfType};
pub use formats::enhanced_nxz::{EnhancedNxzFile, EnhancedNxzHeader};
