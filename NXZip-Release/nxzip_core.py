...
try:
    ...
    SPE_AVAILABLE = True
    print("� SPE Core JIT Engine loaded")
...
class NXZipCore:
    ...
    def __init__(self):
        ...
        print("� NXZip Core v2.0 - 次世代統括圧縮プラットフォーム初期化完了")
        print(f"   TMC Components: {'✅' if TMC_COMPONENTS_AVAILABLE else '❌'}")
        print(f"   SPE Engine: {'✅' if SPE_AVAILABLE else '❌'}")
    ...
    def compress(self, data: bytes, mode: str = "balanced", filename: str = "", 
        ...
        try:
            self.progress_manager.update(5, f"🔥 NXZip {compression_mode.value}モード開始")
            
            # 圧縮パイプライン作成
            pipeline = CompressionPipeline(compression_mode)
            
            self.progress_manager.update(10, "📊 データ解析中...")
            
            # 圧縮実行
            compressed_data, pipeline_info = pipeline.compress(data, encryption_key)
            
            self.progress_manager.update(90, "� 最終処理中...")
            
            compression_time = time.time() - start_time
            compression_ratio = pipeline_info.get('compression_ratio', 0.0)
            
            self.progress_manager.update(100, f"✅ 圧縮完了 - {compression_ratio:.1f}%")
    ...
    def decompress(self, compressed_data: bytes, compression_info: Dict[str, Any]) -> DecompressionResult:
        ...
        try:
            ...
            self.progress_manager.update(10, f"🔍 展開エンジン: {engine}")
            
            # NXZip Core形式の展開
            if engine.startswith('nxzip_core'):
                print(f"🔍 NXZip Core形式として処理開始")
                self.progress_manager.update(20, "🔥 NXZip Core展開中...")
                ...
                if decompressed_data is not None:  # 修正: Noneチェックに変更
                    ...
                    self.progress_manager.update(100, "✅ 展開完了")
            ...
            # フォールバック展開
            self.progress_manager.update(30, f"📂 フォールバック展開: {method}")
